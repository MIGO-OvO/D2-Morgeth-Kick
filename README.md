...
- 自动读取 Destiny 2 客户区尺寸，也可手动设置分辨率。
- 按 15 / 1.00 参考灵敏度换算 ADS 与腰射移动量；FOV 作为游戏设置核对项，不参与相对鼠标计数换算。
- 在“无礼言论”和“通用配枪”之间切换，并分别保存首次转向校准值。
- 调整首次转向、虚空箭落点和冲刺方向。
- 独立恢复默认动作参数或默认键位，互不覆盖。
- 在“入场交互”和“技能终结”之间切换，编辑九段相邻动作等待；支持直接输入和 ±0.05 秒快调。
...
- 主窗口使用与现有控制台风格一致的自定义标题栏，支持自由缩放和最大化；内容会按窗口宽度与 Windows 显示缩放比例自动重排，并在空间不足时提供滚动。
...
3. 固定二号位武器：“无礼言论”模式需装备无礼言论；其他配置使用“通用配枪”，不要换用轻质框架、具有移速效果的异域武器或刀剑。
...
| 瞄点调整 | 首次转向模式、ADS / 腰射独立基准、虚空箭、冲刺方向 | 点击或拖动预览中的蓝色落点，也可输入 XY 数值精确调整；悬停“XY 说明”查看坐标方向 |
| 动作时序 | 入场交互四段、技能终结五段 | 每项表示前一动作完全结束到下一动作开始的等待，范围为 0–10 秒 |

瞄点调整采用屏幕坐标：X 正值向右、负值向左；Y 正值向下、负值向上。Y 轴方向与数学直角坐标系相反。

当前默认配置为“无礼言论”：ADS 基准 `-2600, 50`，虚空箭基准 `-300, 81` 并微调 `-50, 0`，冲刺基准 `280, 0`。入场交互四段默认等待为 `0.600 / 1.000 / 0.200 / 0.500` 秒，技能终结五段为 `1.500 / 0.900 / 0.150 / 2.000 / 0.000` 秒。旧版时序设置会在读取时换算为相邻动作间隔，可通过“恢复默认参数”重新应用整组默认值。
